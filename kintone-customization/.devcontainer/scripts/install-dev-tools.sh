...
if [ ! -e $MKCERT_DIR ]; then
  mkdir -p $MKCERT_DIR
  cd $MKCERT_DIR
  curl -OL $MKCERT_BASE_URL/$MKCERT_VERSIN/$MKCERT_LINUX_BIN
  chmod +x $MKCERT_LINUX_BIN

  curl -OL $MKCERT_BASE_URL/$MKCERT_VERSIN/$MKCERT_MAC_BIN
  chmod +x $MKCERT_MAC_BIN

  curl -OL $MKCERT_BASE_URL/$MKCERT_VERSIN/$MKCERT_WIN_BIN
